...
async function generateKeypair() {
  ...
  console.error('PUBLIC KEY (safe to store in .env.development, .env.test, .env.production)');
  console.error('='.repeat(80));
  console.log(publicKeyPEM);

  console.error('\n' + '='.repeat(80));
  console.error('PRIVATE KEY (NEVER COMMIT - store securely outside repository)');
  console.error('='.repeat(80));
  ...
  console.error('');
  console.error('2. Store the PRIVATE KEY in a secure location OUTSIDE this repository:');
  console.error('   - Local development: ~/.ssh/jwt-private-key.pem or similar');
  console.error('   - Production: Use your platform\'s secret management (e.g., AWS Secrets Manager)');
  console.error('');
  console.error('3. Ensure .gitignore excludes any files containing the private key');
  console.error('='.repeat(80));
}
